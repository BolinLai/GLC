...
class Egteaaction(torch.utils.data.Dataset):
    """
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        assert mode in ["train", "val", "test"], "Split '{}' not supported for Egteaaction".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing Egteaaction {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        """
        if self.mode == 'train':
            path_to_file = 'data/train_split1.csv'
        elif self.mode in ['val', 'test']:
            path_to_file = 'data/test_split1.csv'
        else:
        ...
        with pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                path_label = path_label[1:-1]  # remove the two quotation marks at the beginning and end
                split_items = path_label.split(self.cfg.DATA.PATH_LABEL_SEPARATOR)
                clip, label_action, label_verb = split_items[:3]
                label_nouns = split_items[3:]
                video = '-'.join(clip.split('-')[:3])
                for idx in range(self._num_clips):
                    self._path_to_videos.append(os.path.join(self.cfg.DATA.PATH_PREFIX, 'cropped_clips', video, f'{clip}.mp4'))
                    self._labels.append(int(label_action) - 1)  # shift 1-106 to 0-105
                    self._spatial_temporal_idx.append(idx)
                    self._video_meta[clip_idx * self._num_clips + idx] = {}
        assert (len(self._path_to_videos) > 0), "Failed to load Egteaaction split {} from {}".format(self._split_idx, path_to_file)
        logger.info("Constructing Egteaaction dataloader (size: {}) from {}".format(len(self._path_to_videos), path_to_file))
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ["train"]:
            ...
            min_scale, max_scale, crop_size = ([self.cfg.DATA.TEST_CROP_SIZE] * 3 if self.cfg.TEST.NUM_SPATIAL_CROPS > 1
        ...
        for i_try in range(self._num_retries):
            ...
            if self.aug:
                if self.cfg.AUG.NUM_SAMPLE > 1:
                    ...
                    return frame_list, label_list, index_list, {'path': self._path_to_videos[index]}
            ...
            return frames, label, index, {'path': self._path_to_videos[index], 'index': np.array(frames_global_idx)}
        else:
