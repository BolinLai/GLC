...
def random_crop_gaze(images, size, gaze_loc=None):
    """
    Perform random spatial crop on the given images and corresponding boxes.
    Args:
        images (tensor): images to perform random crop. The dimension is
    ...
    """
    if images.shape[2] == size and images.shape[3] == size:
        return images, gaze_loc
    height, width = images.shape[2], images.shape[3]
    gaze_x, gaze_y = gaze_loc[:, 0] * width, gaze_loc[:, 1] * height
    if width > size:
        sort_gaze_x = np.sort(gaze_x)
        low, high = max(0, sort_gaze_x.max() - size), min(width - size, sort_gaze_x.min())  # try to keep gaze of each frame not cropped
        while low > high:
            sort_gaze_x = sort_gaze_x[1:] if len(sort_gaze_x) % 2 == 0 else sort_gaze_x[:-1]
            low, high = max(0, sort_gaze_x.max() - size), min(width - size, sort_gaze_x.min())
        x_offset = int(low) if low == high else int(np.random.uniform(low, high))
    else:
        x_offset = 0
    if height > size:
        sort_gaze_y = np.sort(gaze_y)
        low, high = max(0, sort_gaze_y.max() - size), min(height - size, sort_gaze_y.min())  # try to keep gaze of each frame not cropped
        while low > high:
            sort_gaze_y = sort_gaze_y[1:] if len(sort_gaze_y) % 2 == 0 else sort_gaze_y[:-1]
            low, high = max(0, sort_gaze_y.max() - size), min(height - size, sort_gaze_y.min())
        y_offset = int(low) if low == high else int(np.random.uniform(low, high))
    else:
        y_offset = 0
    cropped = images[:, :, y_offset: y_offset + size, x_offset: x_offset + size]

    cropped_gaze = gaze_loc.copy()
    cropped_gaze[:, 0] = np.clip((gaze_x - x_offset) / size, a_min=0, a_max=1)  # if gaze is out of patch, use clip on gaze
    cropped_gaze[:, 1] = np.clip((gaze_y - y_offset) / size, a_min=0, a_max=1)  # if gaze is out of patch, use clip on gaze
    return cropped, cropped_gaze
...
def horizontal_flip_gaze(prob, images, gaze_loc=None):
    """
    Perform horizontal flip on the given images and corresponding boxes.
    Args:
        prob (float): probility to flip the images.
    ...
    """
    if gaze_loc is None:
        flipped_gaze_loc = None
    else:
        flipped_gaze_loc = gaze_loc.copy()

    if np.random.uniform() < prob:
        images = images.flip((-1))

        if gaze_loc is not None:
            flipped_gaze_loc[:, 0] = 1 - gaze_loc[:, 0]

    return images, flipped_gaze_loc
...
def uniform_crop_gaze(images, size, spatial_idx, gaze_loc=None, scale_size=None):
    """
    Perform uniform spatial sampling on the images and corresponding boxes.
    Args:
        images (tensor): images to perform uniform crop. The dimension is
