...
class CasGlobalEmbed(nn.Module):
    """
    GlobalEmbed.
    """
    ...
    def __init__(
        ...
        # The length of hidden layer is 96
        self.relu = nn.ReLU()
        self.bn_0 = nn.BatchNorm3d(dim_in)  # it would be better is use synced_batchnorm3d (defined both in this repo and pytorch)
        self.proj_1 = conv(dim_in, dim_in, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1))
        self.bn_1 = nn.BatchNorm3d(dim_in)
        self.proj_2 = conv(dim_in, dim_in, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1))
        self.bn_2 = nn.BatchNorm3d(dim_in)
        self.proj_3 = conv(dim_in, dim_in, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1))
        self.linear = nn.Linear(in_features=dim_in*4*8*8, out_features=96)  # if input is Tx256x256
        # self.linear = nn.Linear(in_features=dim_in*4*7*7, out_features=96)  # if input is Tx224x224

    def forward(self, x, y=None):  # (B, 96, 4, 64, 64)
        # No pooling
        x = self.relu(self.bn_0(x))  # (B, 96, 4, 64, 64)
        x = self.relu(self.bn_1(self.proj_1(x)))  # (B, 96, 4, 32, 32)
        x = self.relu(self.bn_2(self.proj_2(x)))  # (B, 96, 4, 16, 16)
        x = self.proj_3(x)  # (B, 96, 4, 8, 8)
        ...
        x = self.linear(x.view(x.size(0), -1))  # (B, 96)
        return x.unsqueeze(1)  # (B, 1, 96)
