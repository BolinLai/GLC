...
class MViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        temporal_size = cfg.DATA.NUM_FRAMES  # 32
        ...
        # Prepare output.
        num_classes = cfg.MODEL.NUM_CLASSES
        embed_dim = cfg.MVIT.EMBED_DIM  # 768
        ...
        # if we want to fully load MVIT_16x4_K400, uncomment the two rows
        # pool_kv[-2] = [1, 1, 1]
        # pool_kv[-1] = [1, 1, 1]
    ...
    def forward(self, x):
        x = x[0]  # size (B, 3, 8, 224, 224)
        x = self.patch_embed(x)  # size (B, 12544, 96)  12544 = 4*56*56

        T = self.cfg.DATA.NUM_FRAMES // self.patch_stride[0]  # 4
        H = self.cfg.DATA.TRAIN_CROP_SIZE // self.patch_stride[1]  # 56
        W = self.cfg.DATA.TRAIN_CROP_SIZE // self.patch_stride[2]  # 56
        B, N, C = x.shape  # B, 12544, 96
        ...
        for blk in self.blocks:
            x, thw = blk(x, thw)  # x size [B, 197, 768]
